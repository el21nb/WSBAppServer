...
async function createJourneyBusStopDoc() {
    const admin = getAdminFirestore();
...
//BusStopChildren subcollection


//helper functions
function newBusStopChildDoc(thisParentId) {
return busStopChildDoc = {childContacts: myContact, classCode: myClassCode, firstName: myFirstName, id: myChildId, parentId: thisParentId, lastName: myLastName};
}

async function createBusStopChildDoc(thisDriverId, thisParentId) {
    createJourneyDoc(thisDriverId);
    createJourneyBusStopDoc();
    const admin = getAdminFirestore();
    const busStopChildDoc = admin.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await busStopChildDoc.set(newBusStopChildDoc(thisParentId));
}
//Write Tests
...
it("Admin can create a BusStopChild document with correct fields", async() => {
    createJourneyDoc(myDriverId);
    createJourneyBusStopDoc();
    const db = getFirestore(myAdminAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertSucceeds(testDoc.set(newBusStopChildDoc(myParentId)));
});

it("Admin can't create a BusStopChild document with incorrect fields", async() => {
    createJourneyDoc(myDriverId);
    createJourneyBusStopDoc();
    const db = getFirestore(myAdminAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.set({RandomField: 'New time'}));
});

it("Admin can update a BusStopChild document with correct fields", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(myAdminAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertSucceeds(testDoc.update({firstName: 'New Name'}));
});

it("Admin can't update a BusStopChild document with incorrect fields", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(myAdminAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.update({RandomField: 'New time'}));
...
it("Admin can delete a BusStopChild", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(myAdminAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertSucceeds(testDoc.delete());
...
it("Auth user can't create a BusStopChild document", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(myAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.set(newBusStopChildDoc(myParentId)));
});

it("Auth can't update a BusStopChild doc", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(myAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.update({firstName: 'New name'}));
});

it("Unauth user can't create a BusStopChild document", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(null);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.update({firstName: 'New name'}));
...
it("Unauth can't update a BusStopChild", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(null);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.update({firstName: 'New name'}));
...
it("Parent can create their Child's BusStopChild document", async() => {
    createJourneyDoc(myDriverId);
    createJourneyBusStopDoc();
    const db = getFirestore(myParentAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
...
// it("Parent can update their Child's BusStopChild document", async() => {
//     createBusStopChildDoc(myDriverId,myParentId);
//     const db = getFirestore(myParentAuth);
//     const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
//     await firebase.assertSucceeds(testDoc.update({firstName: 'new name'}));
...
// it("Parent can delete their Child's BusStopChild document", async() => {
//     createBusStopChildDoc(myDriverId,myParentId);
//     const db = getFirestore(myParentAuth);
//     const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
//     await firebase.assertSucceeds(testDoc.delete());
...
it("Parent can't create someone elses Child's BusStopChild document", async() => {
    createJourneyDoc(myDriverId);
    createJourneyBusStopDoc();
    const db = getFirestore(theirParentAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
...
it("Parent can't update someone elses Child's BusStopChild document", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(theirParentAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.update({firstName: 'new name'}));
...
it("Parent can't delete someone elses Child's BusStopChild document", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(theirParentAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.delete());
...
it("Admin can't delete a BusStopChild document", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(myAdminAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertSucceeds(testDoc.delete());
...
it("Auth user can't delete a BusStopChild document", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(myAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.delete());
...
it("Unuth user can't delete a BusStopChild document", async() => {
    createBusStopChildDoc(myDriverId,myParentId);
    const db = getFirestore(null);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertFails(testDoc.delete());
...
it("Admin can get a BusStopChild document", async() =>  {
    const db = getFirestore(myAdminAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertSucceeds(testDoc.get());
});

it("Staff can get a BusStopChild document", async() =>  {
    const db = getFirestore(myStaffAuth);
    const testDoc = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren").doc(myChildId);
    await firebase.assertSucceeds(testDoc.get());
});

it("Parent can get their child's BusStopChild document", async() =>  {
     const admin = getAdminFirestore();
    const busStopChildPath = "/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren/id1";
    await admin.doc(busStopChildPath).set(myChildDoc);
    const db = getFirestore(myParentAuth);
    const testDoc = db.doc(busStopChildPath);
    await firebase.assertSucceeds(testDoc.get());
});

it("Parent can't get someone elses child's BusStopChild document", async() =>  {
   const admin = getAdminFirestore();
   const busStopChildPath = "/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren/id1";
   await admin.doc(busStopChildPath).set(myChildDoc);
   const db = getFirestore(theirParentAuth);
...
it("Driver can get a BusStopChild document from their journey", async() =>  {
    const admin = getAdminFirestore();
    const journeyPath = "/Journeys/journeyId1";
    await admin.doc(journeyPath).set(newJourneyDoc(myDriverId));
    const busStopChildPath = "/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren/id1";
...
it("Driver can't get a BusStopChild document from someone else's journey", async() =>  {
    const admin = getAdminFirestore();
    const journeyPath = "/Journeys/journeyId1";
    await admin.doc(journeyPath).set(newJourneyDoc(myDriverId));
    const busStopChildPath = "/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren/id1";
...
it("Admin can list a BusStopChild documents", async() =>  {
    const db = getFirestore(myAdminAuth);
    const testRef = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren");
    await firebase.assertSucceeds(testRef.get());
});

it("Staff can list BusStopChild documents", async() =>  {
    const db = getFirestore(myStaffAuth);
    const testRef = db.collection("Journeys").doc(myJourneyId).collection("JourneyBusStops").doc(myBusStopId).collection("busStopChildren");
    await firebase.assertSucceeds(testRef.get());
});

it("Parent can list their childrens' BusStopChild document", async() =>  {
    const db = getFirestore(myParentAuth);
    const busStopChildrenPath = "/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren";
    const testQuery = db.collection(busStopChildrenPath).where("parentId", "==", myParentId);
    await firebase.assertSucceeds(testQuery.get());
});

it("Parent can't list someone else's childrens' BusStopChild documents", async() => {
    const db = getFirestore(theirParentAuth);
    const busStopChildrenPath = "/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren";
    const testQuery = db.collection(busStopChildrenPath).where("parentId", "==", myParentId);
    await firebase.assertFails(testQuery.get());
...
it("Driver can list BusStopChild documents from their journey", async() =>  {
    const admin = getAdminFirestore();
    const journeyPath = "/Journeys/journeyId1";
    await admin.doc(journeyPath).set(newJourneyDoc(myDriverId));
    
    const db = getFirestore(myDriverAuth);
    const testRef = db.collection("/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren");
    await firebase.assertSucceeds(testRef.get());
});

it("Driver can't list BusStopChild documents from someone else's journey", async() =>  {
    const admin = getAdminFirestore();
    const journeyPath = "/Journeys/journeyId1";
    await admin.doc(journeyPath).set(newJourneyDoc(myDriverId));
...
it("Auth user can't list someone else's childrens' BusStopChild documents", async() => {
    const db = getFirestore(myAuth);
    const testQuery = db.collection("/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren").where("parentId", "==", myParentId);
    await firebase.assertFails(testQuery.get());
});

it("Unauth user can't list someone else's childrens' BusStopChild documents", async() => {
    const db = getFirestore(null);
    const testQuery = db.collection("/Journeys/journeyId1/JourneyBusStops/id1/busStopChildren").where("parentId", "==", myParentId);
    await firebase.assertFails(testQuery.get());
});
